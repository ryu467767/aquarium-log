...
function renderCard(it) {
  ...
  if (it.twitter_id) {
    ...
    xLink.href = `https://x.com/${it.twitter_id}`;
    xLink.target = "_blank";
    xLink.rel = "noopener noreferrer";
    xLink.className = "card-title-x";
    xLink.title = `X: @${it.twitter_id}`;
    xLink.innerHTML = `<svg class="card-title-x-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.73-8.835L1.254 2.25H8.08l4.261 5.635L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg>`;
    titleRow.appendChild(xLink);
  }

  // Xアイコンのすぐ右にInstagramアイコン
  if (it.instagram_id) {
    const igLink = document.createElement("a");
    igLink.href = `https://www.instagram.com/${it.instagram_id}/`;
    igLink.target = "_blank";
    igLink.rel = "noopener noreferrer";
    igLink.className = "card-title-ig";
    igLink.title = `Instagram: @${it.instagram_id}`;
    igLink.innerHTML = `<svg class="card-title-ig-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>`;
    titleRow.appendChild(igLink);
  }
...
// ===== 集めた魚種印（図鑑）モーダル =====
// 訪問済みの水族館に登録された生き物を自動で「ゲット済み」にして図鑑表示する
function openCollectionModal() {
  ...
  const visited = (state.items || []).filter(x => x.visited);

  const dex = CREATURE_DEX.map(c => {
    const aquariums = visited.filter(x => x[c.key]).map(x => x.name);
    return { ...c, count: aquariums.length, aquariums, collected: aquariums.length > 0 };
  });

  // コンプリート表示はしない（要望により削除）
  summary.innerHTML = "";
...
const BADGES = [
  ...
  { id: "a_penguin_3",   label: "ペンギン好き",     icon: "🐧", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_penguin).length >= 3 },
  { id: "a_penguin_5",   label: "ペンギン大好き",   icon: "🐧", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_penguin).length >= 5 },
  { id: "a_penguin_10",  label: "ペンギンマスター", icon: "🐧", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_penguin).length >= 10 },
  { id: "a_penguin_all", label: "ペンギン制覇",     icon: "🐧", cat: "animal",
    check: (items) => { const w = items.filter(x => !x.is_closed && x.has_penguin); return w.length > 0 && w.every(x => x.visited); } },
  { id: "a_dolphin_3",   label: "イルカ好き",       icon: "🐬", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_dolphin).length >= 3 },
  { id: "a_dolphin_5",   label: "イルカ大好き",     icon: "🐬", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_dolphin).length >= 5 },
  { id: "a_dolphin_10",  label: "イルカマスター",   icon: "🐬", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_dolphin).length >= 10 },
  { id: "a_dolphin_all", label: "イルカ制覇",       icon: "🐬", cat: "animal",
    check: (items) => { const w = items.filter(x => !x.is_closed && x.has_dolphin); return w.length > 0 && w.every(x => x.visited); } },
  { id: "a_sealion_3",   label: "アシカ好き",       icon: "🦭", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_sealion).length >= 3 },
  { id: "a_sealion_5",   label: "アシカ大好き",     icon: "🦭", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_sealion).length >= 5 },
  { id: "a_sealion_10",  label: "アシカマスター",   icon: "🦭", cat: "animal",
  ...
  { id: "a_jellyfish_5",   label: "クラゲ大好き",   icon: "🪼", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_jellyfish).length >= 5 },
  { id: "a_jellyfish_10",  label: "クラゲマスター", icon: "🪼", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_jellyfish).length >= 10 },
  ...
  { id: "a_shark_3",   label: "サメ好き",       icon: "🦈", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_shark).length >= 3 },
  { id: "a_shark_5",   label: "サメ大好き",     icon: "🦈", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_shark).length >= 5 },
  { id: "a_shark_10",  label: "サメマスター",   icon: "🦈", cat: "animal",
    check: (items) => items.filter(x => !x.is_closed && x.visited && x.has_shark).length >= 10 },
  { id: "a_shark_all", label: "サメ制覇",       icon: "🦈", cat: "animal",
    check: (items) => { const w = items.filter(x => !x.is_closed && x.has_shark); return w.length > 0 && w.every(x => x.visited); } },
  // シロイルカ（3館）
  { id: "a_beluga_all", label: "シロイルカ制覇", icon: "🐳", cat: "animal",
    check: (items) => { const w = items.filter(x => !x.is_closed && x.has_beluga); return w.length > 0 && w.every(x => x.visited); } },
];
...
async function load() {
  ...
  initMap();
  colorJapanMap();   // 都道府県タブが読み込み済みなら塗り直す
...
function wireMapTabs() {
  ...
  const prefPane = document.getElementById('prefPane');
  if (!tabs.length || !leafletPane || !prefPane) return;

  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      tabs.forEach(t => t.classList.toggle('is-active', t === tab));
      if (tab.dataset.maptab === 'pref') {
        leafletPane.hidden = true;
        prefPane.hidden = false;
        showJapanMap();
      } else {
...
function prefStats(pref) {
  const list = (state.items || []).filter(x => x.prefecture === pref && !x.is_closed);
  return { total: list.length, visited: list.filter(x => x.visited).length, list };
}

async function showJapanMap() {
  const host = document.getElementById('japanMap');
  if (!host) return;

  if (!japanMapLoaded) {
    try {
      const res = await fetch('japan.svg');
      host.innerHTML = await res.text();
      japanMapLoaded = true;
      host.querySelectorAll('g.prefecture').forEach(g => {
        g.style.cursor = 'pointer';
        g.addEventListener('click', () => {
          const code = parseInt(g.getAttribute('data-code'), 10);
          showPrefDetail(PREF_ORDER[code - 1]);
        });
      });
    } catch (e) {
      host.innerHTML = '<div class="japan-map-err">地図の読み込みに失敗しました</div>';
      return;
    }
  }
  applyJapanZoom();
...
function colorJapanMap() {
  const host = document.getElementById('japanMap');
  if (!host || !japanMapLoaded) return;
  host.querySelectorAll('g.prefecture').forEach(g => {
    const code = parseInt(g.getAttribute('data-code'), 10);
    const { total, visited } = prefStats(PREF_ORDER[code - 1]);
    let color;
    if (total === 0) color = '#eceff1';            // 施設なし
    else if (visited === 0) color = '#cfd8dc';     // 未訪問
    else if (visited < total) color = '#ffd54f';   // 一部訪問
    else color = '#4db6ac';                        // 制覇
